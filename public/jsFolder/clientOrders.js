...
const getAllMenuProductFunc = async () => {
  ...
  try {
    const getAllMenuProductResponse = await fetch(
      `${config2.apiUrl}/doveeysKitchen/product/getMenuProducts`,
    );
...
const fetchSingleProductFunc = async (menuProductId) => {
  try {
    const fetchSingleProductResponse = await fetch(
      `${config2.apiUrl}/doveeysKitchen/product/getSingleMenuProduct/${menuProductId}`,
    );
...
const populateUserProceedOrder = () => {
  ...
  if (
    ...
    (!menuProductVariations ||
      ...
      isAllVariationsInvalidMenuPrice(menuProductVariations))
  ) {
    priceDisplay = `
        <div class="mb-4">
  ...
  function isAllVariationsInvalidMenuPrice(menuProductVariations) {
    return menuProductVariations.every(
      (menuProductVariation) => !menuProductVariation.size || menuProductVariation.price === null,
    );
  }
  ...
  proceedButton.addEventListener("click", async (e) => {
    ...
    const selectedSizeValue = variationSelect
      ...
      : menuProductVariations[0]?.size || "Default";
...
const userProceedOrderFunc = async (formData) => {
  ...
  try {
    const userProceedResponse = await fetch(
      `${config2.apiUrl}/doveeysKitchen/order/createProceedOrder`,
      {
        method: "POST",
        headers: {
    ...
    document.getElementById("menuOrderLoader").classList.add("hidden");
  }
};
...
const initializeEventListeners = (proceedOrderPrice) => {
  ...
  if (variationSelect) {
    variationSelect.addEventListener("change", (event) => {
      ...
      document.getElementById("orderProceedPrice").innerHTML =
...
const handleMakePay = async (formData) => {
  try {
    const res = await fetch(`${config2.apiUrl}/api/paystack/init`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function verifyPayment(reference, formData) {
  try {
    const verifyRes = await fetch(`${config2.apiUrl}/api/paystack/verify`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
